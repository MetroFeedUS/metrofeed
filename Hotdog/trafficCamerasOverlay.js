...
const TrafficCamerasOverlay = (function() {
  ...
  async function loadCamerasData() {
    ...
    try {
      ...
      camerasData = Array.isArray(list) ? list.map((c, idx) => {
        ...
        if (mapped.url) {
          mapped.url = String(mapped.url).replace(
            '://itscameras.dot.state.oh.us:443/',
  ...
  /**
   * Build direct + optional MetroFeed proxy image URLs for an Ohio camera.
   * Inventory often points at itscameras.dot.state.oh.us; proxy is
   * /cameras/ohio/{id}/image on traffic-api (when deployed).
   */
  function resolveCameraImageUrls(camera) {
    let direct = String(
      (camera && (camera.url || camera.imageUrl || camera.image_url || camera.streamUrl || camera.stream_url)) || ''
    ).trim();
    // Normalize odd :443 in https URLs from the inventory feed
    if (direct) {
      direct = direct.replace('://itscameras.dot.state.oh.us:443/', '://itscameras.dot.state.oh.us/');
    }
    const id = camera && camera.id != null ? String(camera.id) : '';
    const feed =
      typeof window !== 'undefined' && window.CITY_CONFIG && window.CITY_CONFIG.camerasFeedUrl
        ? String(window.CITY_CONFIG.camerasFeedUrl)
        : '';
    const proxy =
      feed.indexOf('ohio') >= 0 && id
        ? 'https://traffic-api.metrofeedus.com/cameras/ohio/' + encodeURIComponent(id) + '/image'
        : '';
    return { direct: direct, proxy: proxy };
  }
  ...
  function showCameraFeed(cameraId, cameraName, imageUrl) {
    ...
    const resolved = resolveCameraImageUrls({ id: cameraId, url: imageUrl });
    const primary = resolved.direct || resolved.proxy;
    if (!primary) {
      if (modalAttribution) {
        modalAttribution.textContent = 'Camera feed not available for this location.';
        modalAttribution.style.color = '#f59e0b';
      }
    ...
    if (modalAttribution) {
      ...
      modalAttribution.style.color = '';
    }

    let triedProxy = false;
    modalImage.onerror = function () {
      if (resolved.proxy && !triedProxy && primary !== resolved.proxy) {
        triedProxy = true;
        console.warn('[TrafficCamerasOverlay] Direct camera URL failed; trying traffic-api proxy');
        modalImage.src = resolved.proxy + '?t=' + Date.now();
        return;
      }
      modalImage.style.display = 'none';
      if (modalAttribution) {
        modalAttribution.textContent =
          'Camera image unavailable right now. OHGO/ODOT snapshots may be down, or the MetroFeed image proxy needs to be enabled on traffic-api.';
        modalAttribution.style.color = '#f59e0b';
      }
      console.error('[TrafficCamerasOverlay] Camera image failed to load:', cameraId, primary);
    };
    ...
    const separator = primary.indexOf('?') >= 0 ? '&' : '?';
    modalImage.src = primary + separator + 't=' + timestamp;
