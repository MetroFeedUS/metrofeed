...
(function () {
  ...
  const CITY =
    (window.CITY_CONFIG && window.CITY_CONFIG.cityName) || 'Test City';
  const STATE = (window.CITY_CONFIG && window.CITY_CONFIG.state) || 'KY';
  const citySlug =
    typeof getCityIdFromPath === 'function' ? getCityIdFromPath() : 'testcity';

  const TIPS = [
    'Live buses & routes at roamravenapp.com/' + citySlug,
    'Tap a route on the map for stops, times & alerts',
    'Test sandbox · Cincinnati transit data · map centered on Lexington',
